...
app.get('/products', async (req, res) => {
    try {
        ...
        // **1️⃣ Create Mappings for Categories & Vendors**
        const categoryMap = {};
        const vendorMap = {};
        const imageMap = {};
        ...
        data.objects.forEach(obj => {
            if (obj.type === "CATEGORY" && obj.category_data) {
                categoryMap[obj.id] = obj.category_data.name; // Store category ID → category name
            } else if (obj.type === "VENDOR" && obj.vendor_data) {
                vendorMap[obj.id] = obj.vendor_data.name; // Store vendor ID → vendor name
            } else if (obj.type === "IMAGE" && obj.image_data) {
                imageMap[obj.id] = obj.image_data.url; // Store image ID → image URL
            }
        });

        // **2️⃣ Extract & Format Product Data**
        const formattedProducts = data.objects
            ...
            .map(item => {
                ...
                // **3️⃣ Get Category & Vendor Names Using Mappings**
                const categoryName = item.item_data.category_id
                    ? categoryMap[item.item_data.category_id] || "Uncategorized"
                    : "Uncategorized";

                const vendorName = item.item_data.vendor_ids?.[0]
                    ? vendorMap[item.item_data.vendor_ids[0]] || "Unknown Vendor"
                    : "Unknown Vendor";
                ...
                return {
                    ...
                    category: categoryName, // ✅ Now uses real category names
                    vendor: vendorName, // ✅ Now uses real vendor names
